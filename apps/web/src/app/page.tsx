...
export default function HomePage() {
  ...
  if (!data) return <main className="connection-state"><p>{error ?? "로그인이 필요합니다."}</p><a className="primary-button" href={`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}/api/auth/github`}>GitHub로 로그인</a></main>;
